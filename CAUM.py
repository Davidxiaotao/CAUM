    ...
    if error_UO2.get().replace('.', '', 1).isdigit() and error_ThO2.get().replace('.', '', 1).isdigit() and error_PbO.get().replace('.', '', 1).isdigit():# To determine whether the error format of UO2, ThO2 and PbO entered by the user is correct.
        ...
        if 'PbO' in df.columns and ('UO2' in df.columns or 'ThO2' in df.columns):
            if 0 in df['PbO'].values.tolist():
                ...
                list_vaild = list(zip(list_ThO2, list_UO2, list_PbO))  # The corresponding oxide package forms a list
                ...
                df['Age(Ma)'] = df['Age(Ma)'].apply(lambda x: round(x, 2))  # The apparent age retains two decimal places
                ...
                df['Error(Ma)'] = df['Error(Ma)'].apply(lambda x: round(x, 2))  # The apparent age error retains two decimal places
                df_datashow = df
...
def weighted_age():
    ...
    def average_age():
        ...
        if 'Age(Ma)' in df_1.columns and 'Error(Ma)' in df_1.columns:
            ...
            a.set_ylabel('Age(Ma)', fontdict=dict(weight='medium', family='Times New Roman'))  # Set the Y-axis label
        ...
        if len(show_file.get()) == 0:
            ...
            if len(X_element.get()) == 0 or len(Y_element.get()) == 0:
                ...
                for label in ax.get_xticklabels():
                    label.set_fontname('Times New Roman')
                # Modify the Y-axis scale font to Times New Roman
