...
def Run():
    ...
    for widget in frame3.winfo_children():
        widget.destroy()
    if error_UO2.get().replace('.', '', 1).isdigit() and error_ThO2.get().replace('.', '', 1).isdigit() and error_PbO.get().replace('.', '', 1).isdigit():
        global df
        df = pd.read_excel(file)
        if 'PbO' in df.columns and ('UO2' in df.columns or 'ThO2' in df.columns):
            if 0 in df['PbO'].values.tolist():
                tkinter.messagebox.showinfo('Message','The percentage of PbO content at each analysis point can\'t be zero',parent=win)
            else:
                if 'ThO2' in df:
                    pass
                else:
                    example_list = [0, ] * len(df)
                    df['ThO2'] = np.array(example_list)  # dataframe中插入ThO2含量
                W_Th = 264
                W_U = 270
                W_Pb = 222
                a1 = 0.00000000098485  # U235的衰变常数
                b1 = 0.000000000155125  # U238的衰变常数
                r232 = 0.000000000049475  # Th232的衰变常数
                ThO2 = df['ThO2']
                UO2 = df['UO2']
                PbO = df['PbO']
                list_ThO2 = ThO2.values.tolist()  # series转列表
                list_UO2 = UO2.values.tolist()  # series转列表
                list_PbO = PbO.values.tolist()  # series转列表
                list_vaild = list(zip(list_ThO2, list_UO2, list_PbO))  # 对应打包元素构成一个列表
                list_age = []  # 创建空列表存入迭代后年龄
                for i in list_vaild:
                    i = list(i)
                    t = i[2] * 100 * 1e6
                    while ((i[0] / W_Th) * (math.exp(r232 * t) - 1) + (i[1] / W_U) * (
                            ((math.exp((a1) * t) + 137.88 * math.exp((b1) * t)) / 138.88) - 1)) > i[2] / W_Pb:
                        t = t - 1e4  # 以0.01Ma为单元重复迭代
                    t = t * 1e-6  # 转化为以Ma为单位
                    list_age.append(t)  # 各点视年龄存入空列表
                df['Age(Ma)'] = np.array(list_age)  # dataframe中插入视年龄
                df['Age(Ma)'] = df['Age(Ma)'].apply(lambda x: round(x, 2))  # 年龄保留两位小数
                # 计算单点年龄误差
                ...
                if isFloat(error_UO2.get()) == True and isFloat(error_ThO2.get()) == True and isFloat(
                    ...
                    for i in list(zip(list_ThO2, list_UO2, list_PbO, list_age)):
                        ...
                        K_PbO = (1 / 222) / ((r232 * i[0] * math.exp(r232 * i[3]) / 264) + (
                                a1 * i[1] * math.exp(a1 * i[3]) + 137.88 * b1 * i[1] * math.exp(b1 * i[3])) / (
                                                     270 * 138.88))
                        dev_UO2 = float(error_UO2.get()) / 100  # 获取dev_UO2的值
                        dev_ThO2 = float(error_ThO2.get()) / 100  # 获取dev_ThO2的值
                        dev_PbO = float(error_PbO.get()) / 100  # 获取dev_PbO的值
                        # 计算年龄绝对误差Δt
                        abs_t = K_UO2 * abs(i[1] * dev_UO2) + K_ThO2 * abs(i[0] * dev_ThO2) + K_PbO * abs(
                            i[2] * dev_PbO)
                        abs_t = abs_t * 1e-6  # 转化为以Ma为单位
                        list_error_t.append(abs_t)
                df['Error(Ma)'] = np.array(list_error_t)  # dataframe中插入单点年龄误差
                df['Error(Ma)'] = df['Error(Ma)'].apply(lambda x: round(x, 2))  # 单点年龄误差保留两位小数
                df_datashow = df
                df_datashow['ID'] = np.array(list(range(1, len(df_datashow) + 1)))
                df = df.drop(['ID'], axis=1)  # 去除名为‘ID’的列
                ...
                # 计算相关系数
                df_datashow = df_datashow.drop(['ID'], axis=1)  # 去除名为‘ID’的列
                if 'Total' in df_datashow:
                    df_datashow = df_datashow.drop(['Total'], axis=1)  # 去除名为‘Total’的列
                if 'Age(Ma)' in df_datashow:
                    df_datashow = df_datashow.drop(['Age(Ma)'], axis=1)  # 去除名为‘Age(Ma)’的列
                if 'Error(Ma)' in df_datashow:
                    df_datashow = df_datashow.drop(['Error(Ma)'], axis=1)  # 去除名为‘Error(Ma)’的列
                # 计算每两列之间的相关系数
                # 迭代计算P值和R值
                ...
                for col1 in correlation.columns:
                    for col2 in correlation.columns:
                        if col1 != col2:
                            if df_datashow[col1].nunique() > 1 and df_datashow[col2].nunique() > 1:  # 排除常数列
                                r, p = pearsonr(df_datashow[col1], df_datashow[col2])
                                r = format(r, '.9f')  # 保留9位小数
                                p = format(p, '.9f')  # 保留9位小数
                ...
                # 添加标题和标签
                ax1.set_xlabel('Age(Ma)', fontdict=dict(weight='medium', family='Times New Roman'))
                ax1.set_ylabel('Number', fontdict=dict(weight='medium', family='Times New Roman'))
                # 创建第二个Y轴用于绘制核密度曲线
                ax2 = ax1.twinx()
                # 添加概率密度曲线
                sns.kdeplot(list_age, color='red', ax=ax2, linewidth=3)
                # 隐藏图形 Y 轴右侧的标签和坐标轴
                ax2.set_ylabel('')  # 隐藏右侧 Y 轴标签
                ax2.set_yticks([])  # 隐藏右侧 Y 轴标签
                ax2.spines['right'].set_visible(False)  # 隐藏右侧 Y 轴坐标轴线
                # 修改X轴刻度的字体为Times New Roman
                for label in ax1.get_xticklabels():
                    label.set_fontname('Times New Roman')
                # 修改Y轴刻度的字体为Times New Roman
                for label in ax1.get_yticklabels():
                    label.set_fontname('Times New Roman')
                # 将Matplotlib图形嵌入到Tkinter
                canvas1 = FigureCanvasTkAgg(fig1, master=frame3)
                canvas1.draw()
                canvas1.get_tk_widget().pack(side=tkinter.TOP, fill=tkinter.BOTH, expand=tkinter.YES)  # 随窗口大小调整而调整
...
def weighted_age():
    ...
    def average_age():
        ...
        if 'Age(Ma)' in df_1.columns and 'Error(Ma)' in df_1.columns:
            ...
            MSWD = sum(mswd)
            average_t = '{:.2f}'.format(average_t)  # 年龄保留一位小数
            total_error = '{:.2f}'.format(total_error)  # 误差保留一位小数
            MSWD = '{:.2f}'.format(MSWD)  # 误差保留一位小数
            polit = chi2.sf(float(MSWD) * (len(list_age) - 1), len(list_age) - 1)
            # 清楚上一次图片和结果
            Result_age.delete(0, END)
            Result_error.delete(0, END)
            Result_MSWD.delete(0, END)
            ...
            for widget in Weight_average.winfo_children():
                widget.destroy()
            # 显示结果
            Result_age.insert(INSERT, average_t)
            Result_error.insert(INSERT, total_error)
            Result_MSWD.insert(INSERT, MSWD)
            Result_P.insert(INSERT, polit)
            # 绘制加权平均图
            f = Figure(figsize=(6, 5), dpi=110)
            a = f.add_subplot(111)
            y = df_1['Age(Ma)'].values.tolist()  # series转列表
            x = list(range(1, len(y) + 1))
            yerr = df_1['Error(Ma)'].values.tolist()  # series转列表
            a.errorbar(x, y, yerr, fmt='co', mec='red', elinewidth=8, ecolor='red', mfc='red', ms=1)  # 绘制误差图
            a.axhline(y=float(average_t), c='green', lw=2, zorder=0)  # 绘制平均年龄直线
            a.tick_params(axis='x', bottom=False, colors='white')  # X轴坐标隐藏
            a.tick_params(axis='y', tickdir='in')  # Y轴刻度指向图内
            a.set_ylabel('Age(Ma)', fontdict=dict(weight='medium', family='Times New Roman'))  # 设置Y轴标签
            a.set_xlim(0, len(y) + 2)  # 设置X轴范围
            a.set_ylim(min(y) - 2 * yerr[y.index(min(y))], max(y) + 2 * yerr[y.index(max(y))])  # 设置Y轴范围
            a.set_facecolor('#DCDCDC')  # 设置加权图的背景色
            a.grid(axis='y')  # 添加Y轴网格线
            # 设置右上角注释（加权图的绘制类型：1σ或2σ）
            if vali.get() == 1:
                a.set_title('box heights are ' + str(1) + 'σ', loc='right',
                            fontdict=dict(weight='medium', family='Times New Roman', size=10), pad=5)
            else:
                a.set_title('box heights are ' + str(2) + 'σ', loc='right',
                            fontdict=dict(weight='medium', family='Times New Roman', size=10), pad=5)
            # 加权平均年龄图中插入年龄文本
            a.text(0.83, 0.92, s='Mean=' + str(average_t) + '±' + str(total_error) + 'Ma' + '\nMSWD=' + str(
                MSWD), fontdict=dict(weight='medium', family='Times New Roman'),
            ...
            canvas.draw()
            canvas.get_tk_widget().pack(side=tkinter.TOP, fill=tkinter.BOTH, expand=tkinter.YES)  # 随窗口大小调整而调整
...
def estimate_age():
    ...
    def clear():
        show_file.delete(0, END)  # 清空显示路径文本框
        plot_title.delete(0, END)  # 清空图形标题
        X_plot_title.delete(0, END)  # 清空X轴标签
        Y_plot_title.delete(0, END)  # 清空Y轴标签
        X_element.delete(0, END)  # 清空X轴标签
        Y_element.delete(0, END)  # 清空X轴标签
        X_element.values = []
        Y_element.values = []
        set_bands.delete(0, END)  # 清空置信参数
        result_slope.delete(0, END)  # 清空斜率
    ...
    def plot():
        ...
        if len(show_file.get()) == 0:
            ...
            if 'ThO2' in df_plot:
                ...
                df_plot['ThO2'] = np.array(example_list)  # dataframe中插入ThO2含量
            if 'Age(Ma)' in df_plot:
                ...
                for i in list_vaild:
                    ...
                    while ((i[0] / W_Th) * (math.exp(r232 * t) - 1) + (i[1] / W_U) * (
                        ...
                        t = t - 1e4  # 以0.01Ma为单元重复迭代
                    t = t * 1e-6  # 转化为以Ma为单位
                    # t = '{:.2f}'.format(t)                       # 保留两位小数
                    list_age.append(t)  # 各点视年龄存入空列表
                df_plot['Age(Ma)'] = np.array(list_age)  # dataframe中插入视年龄

            if len(X_element.get()) == 0 or len(Y_element.get()) == 0:
                tkinter.messagebox.showinfo('Message', 'x(y) parameter cannot be null', parent=t3)
            else:
                X_list = X_element.get().split('+')  # 获取X轴所需绘制的氧化物列表
                Y_list = Y_element.get().split('+')  # 获取X轴所需绘制的氧化物列表
                ...
                ax.set_xlabel(X_plot_title.get(), fontdict=dict(weight='medium', family='Times New Roman'))
                ax.set_ylabel(Y_plot_title.get(), fontdict=dict(weight='medium', family='Times New Roman'))
                ax.tick_params(axis='x', tickdir='in')  # x轴刻度指针向里
                ax.tick_params(axis='y', tickdir='in')  # Y轴刻度指针向里
                ax.set_title(plot_title.get(), fontdict=dict(weight='medium', family='Times New Roman'),
                             size=10, loc='center')
                ax.set_title('Slope: {:.7f}'.format(b) + '\nIntercept: {:.7f}'.format(a),
                             fontdict=dict(weight='medium', family='Times New Roman', size=10), loc='left')
                ax.set_title('R2: {:.7f}'.format(r2_value) + '\nP: {:.7f}'.format(P_value),
                             fontdict=dict(weight='medium', family='Times New Roman', size=10), loc='right')
                ...
                if Confidence_Bands.get():
                    ...
                    try:
                        if float(bands_params) >= 0 and float(bands_params) <= 1:
                            ...
                            if Confidence_Bands.get():  # 如果置信区间被勾选
                                # 计算Y轴截距的置信区间的边界值
                            ...
                            ax.legend(loc='best',
                                      prop={'weight': 'medium', 'family': 'Times New Roman', 'size': 'medium'})
                            # 创建一个Figure对象并将其放入Tkinter窗口
                            fig_canvas = FigureCanvasTkAgg(fig, master=labelframe_Age)
                            fig_canvas.draw()
                            fig_canvas.get_tk_widget().pack()
                            # 创建一个工具栏对象并将其放入Tkinter窗口
                    ...
                    ax.legend(loc='best', prop={'weight': 'medium', 'family': 'Times New Roman', 'size': 'medium'})
                    # 创建一个Figure对象并将其放入Tkinter窗口
                    fig_canvas = FigureCanvasTkAgg(fig, master=labelframe_Age)
                    fig_canvas.draw()
                    fig_canvas.get_tk_widget().pack()
    ...
    # 设置年龄推算图区域
    labelframe_Age = tkinter.LabelFrame(t3, text='Diagram of cocrrelation', width=640, height=550, font='Times 17 bold')
...
def Mix_age():
    def count_sub_age_domain(data, n, sigma):
        data = sorted(data, key=lambda x: x[0])  # 对数据按照从小到大排列
        # 生成划分方案的索引组合
        ...
        def countssa(list1):  # 计算每个子年龄域SSE
            age_up = []
            age_down = []
            for i in list1:
                age_up.append(i[0] / (i[1] ** 2))
        ...
        for i in result:
            sub_sse = 0
            for j in i:
                sub_sse += j[-1]  # 累加最后一个元素
            sub.append(sub_sse)
        vaild_index = sub.index(min(sub))
        vaild_age_domain = result[vaild_index]  # 有效区分的子年龄结果
        # 计算每个子年龄域的年龄
        total_result = []
    ...
    def clear_all():
        show_file.delete(0, END)  # 清除文件路径显示框
        # 清除所有Age结果
    ...
    def count():# 流程：读取文件-获取参数-计算结果-结果显示至gui界面
        df_mix = pd.read_excel(file3)  # 表格中必须包含年龄和误差
    ...
    spin.insert(0, 1)  # 设置初始值为1
    spin.grid(row=16, column=0, pady=5, padx=5, columnspan=2, sticky=E)  # sp.get()用于获取当前的值
    # 设置‘calculate age’的按键
    ...
    tkinter.Label(Result, text='N', font='Times 15 bold').grid(row=1, column=4, padx=10, pady=5)
win = tkinter.Tk()
plt.rcParams['font.sans-serif'] = ['SimHei']  # 用来解决正常不能正常显示的问题
plt.rcParams['axes.unicode_minus'] = False
stlyle = ttk.Style(theme='cosmo')
win.title('Copyright © 2023 CAUM. All Rights Reserved')
ctypes.windll.shcore.SetProcessDpiAwareness(1)  # 解决窗口缩放问题
...
# # 设置表头字体
style = ttk.Style()
style.configure("Treeview.Heading", font='Times 15 ')  # 修改表头字体
style.configure('Treeview', font='Times 15 ', rowheight=30)  # 修改表格内容字体
